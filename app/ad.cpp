...
CDevice_ad::CDevice_ad(const char *pname, uint16 oflag):CDevice_base(pname, oflag)
{
...
CDevice_ad::~CDevice_ad()
{
    memset(m_data, 0, sizeof(m_data));
}

portBASE_TYPE CDevice_ad::process_read(enum PROC_PHASE phase, char *pbuf, portSIZE_TYPE size)
{
	switch (phase){
	case PROC_PREPARE:
		break;
